/** Convex hull geometry for the cluster outlines. Screen space: y points
 *  down, so "clockwise" below means clockwise as drawn, not as a
 *  mathematician would have it. */
...
/** Traces the hull expanded outwards by `pad`, with the corners rounded to that same radius. */
...
/** Y of the ring's upper boundary directly above `x`. */
...
/** For the Haru Urara theme a cluster is not outlined by its hull at all: it is enclosed by a… */

/**
 * The outer edge, away from the notch: a superellipse about the polar centre, with its own reach…
 */
...
/** The notch, cut by a straight chord on each side running from the valley out to a tip. */
...
/** The profile, tabulated, for the fit to read. */
const PETAL_SLOTS = 2048
/** Pieces each slot is cut into before its minimum is taken. */
...
/**
 * How far the ring's boundary lies from `c` along the ray at `dir`, or -1 if the ray misses it --…
 */
...
/** Smallest petal that holds `ring` with at least `pad` clearance, over the rotations tried. */
...
function fitAt(
  ...
  for (let k = 0; k < PETAL_TURNS; k++) {
    ...
    // The valley, where the profile is at its lowest and kinked, is the one direction a fit is
    // most…
...
/** Y of the outline directly above the centre. */
