 ...
 *
 * Two kinds of source feed this. Eight paintings come from the upstream M3
 * token sets in github.com/peiqingzhang/art_inspired_design_system_for_AI; the
 * other two, "Haru Urara" and "Studio Paper", are the app's own palettes,
 * defined below and put through exactly the same pipeline so they carry the
 * same guarantees.
 *
...
/**
 * The app's namesake palette, and its default.
 *
 * Haru Urara's colours: sakura pink, a clear spring sky, and the sunlit gold
 * of her accessories, on blossom-tinted paper. Corners are rounder than any
 * other theme and the display face is a rounded gothic, because the whole
 * point of naming the tool after her is that a page full of documentation
 * defects should not feel like a scolding.
 *
 * The pink is deepened well past her actual hair colour. A fact node is a
 * ten-pixel square and `--fact` doubles as the accent behind buttons and
 * focus rings, so it has to clear 4:1 on white; a true sakura pink lands
 * around 1.6:1 and would have been pushed there by the contrast pass anyway.
 * Fixing it here keeps the hue relationships deliberate rather than derived.
 */
const HARU_URARA = {
  id: 'haru-urara',
  name: 'Haru Urara',
  subtitle: 'Sakura pink & spring sky',
  kind: 'house',
  colors: {
    primary: '#d11d72',
    'on-primary': '#ffffff',
    // Sky and gold. The mapping below hands the dimension role to whichever of
    // these two sits further from the pink -- the sky -- and the gold becomes
    // the conformed marker.
    //
    // Gold lands 15 away from the amber warning, which is closer than any
    // other theme manages. It is kept anyway because the two never share a
    // surface: conformed is a node border on the canvas, where its neighbours
    // are the fact (83), the dimension (101) and danger (60), while warning is
    // a chip in the diagnostics pane. The obvious alternative, a burnt orange,
    // buys separation from warning only by closing on danger (37) -- a
    // collision on the canvas, which is the surface that matters.
    secondary: '#2f8fa6',
    tertiary: '#c07d09',
    error: '#c0182e',

    // Blossom paper, run on the same plan as Studio Paper: `surface` is the
    // page and the graph canvas, containers run lighter to the panels, and
    // `highest` is deliberately darker than the page so a hovered row on a
    // white panel still reads as recessed.
    surface: '#fdf1f5',
    'on-surface': '#2a1720',
    'surface-variant': '#f2d9e3',
    'on-surface-variant': '#6d4a58',
    'surface-container-highest': '#f9e7ee',
    'surface-container-high': '#fdf1f5',
    'surface-container': '#fef7fa',
    'surface-container-low': '#ffffff',
    'surface-container-lowest': '#ffffff',

    outline: '#e0bccc',
    'outline-variant': '#f0dbe4',
    'on-primary-container': '#2a1720',
    'inverse-on-surface': '#fef7fa',
    scrim: '#000000',
    // A plum shadow rather than a black one, so panels lifting off blossom
    // paper do not cast a grey that belongs to no other colour on the page.
    shadow: '#4a1b33',
  },
  corner: {
    'extra-small': '6px', small: '10px', large: '16px', 'extra-large': '22px', full: '9999px',
  },
  fonts: { brand: 'M PLUS Rounded 1c', plain: 'Nunito Sans' },
}
...
// The house palettes lead, namesake first; the paintings follow alphabetically.
const sources = [HARU_URARA, STUDIO_PAPER, ...slugs.map(readPainting)]
 ...
 * "Haru Urara" and "Studio Paper" are the app's own palettes, defined in the
 * generator.
