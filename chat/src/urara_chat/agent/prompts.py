...
# A human turn, not a system one: the Google adapter hoists system messages to the front of the
# context, away from the work this is about.
TOOL_BUDGET_SPENT = (
...
# What a tool call is answered with once the budget is gone.
TOOL_BUDGET_SPENT_RESULT = "Not run: the tool budget for this turn is spent."

# The reader's answer when the model never wrote one -- it spent its whole budget asking for tools
# and asked again on its final pass.
NO_ANSWER_PRODUCED = (
...
# document that reads like an instruction from becoming one.
_UNTRUSTED_FENCE = (
...
# The prompt names the language rather than its code.
