...
class AgentState(TypedDict):
    ...
    iterations: int
    truncated: bool
    # Tracked explicitly rather than inferred from the iteration count: the post-budget pass must
    # run exactly once, and a count can be reasoned about wrongly by anyone who later changes when
    # it is incremented.
...
def build_graph(
    ...
    async def load_context(state: AgentState) -> dict[str, Any]:
        ...
        # add_messages appends, so prepending means replacing the list: the system prompt has to
        # be first or the model reads the question before its instructions.
        return {
            "context_card": card,
    ...
    async def finalise(state: AgentState) -> dict[str, Any]:
        """Assemble the answer and work out what it rested on."""
    ...
    # Unconditionally back to the agent for one final pass, then out: the flag set above is what
    # stops that pass from routing into the loop again.
